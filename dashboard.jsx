...
const INITIAL_DB = {
  admins: [
    { id: 'A1', name: 'Super Administrator', email: 'admin@admin.com', password: 'admin123' }
  ],
  students: [
    { 
      id: 'S1', name: 'Alice Smith', rollNo: 'CS-2024-001', section: 'A', photo: 'https://i.pravatar.cc/150?u=S1', password: 'password123',
      personalDetails: { email: 'alice@student.com', phone: '+1 234 567 8900', parentPhone: '+1 234 567 8999', address: '123 Campus Drive, Block B', dob: '2004-05-14' },
      collegeDetails: { 
        course: 'B.Tech', branch: 'Computer Science', year: '2nd Year', batch: '2024-2028', semester: '4th', advisor: 'Prof. John Doe',
        faculties: [
          { subject: 'Data Structures', name: 'Prof. John Doe' },
          { subject: 'Algorithms', name: 'Prof. Alan Turing' },
          { subject: 'Database Systems', name: 'Prof. Ada Lovelace' }
        ]
      },
      attendance: [
        { date: '2026-03-20', subject: 'Data Structures', status: 'Present', time: '09:00 AM' },
        { date: '2026-03-20', subject: 'Algorithms', status: 'Present', time: '10:00 AM' },
        { date: '2026-03-20', subject: 'Database Systems', status: 'Absent', time: '11:30 AM' },
        { date: '2026-03-21', subject: 'Data Structures', status: 'Present', time: '09:00 AM' },
        { date: '2026-03-21', subject: 'Algorithms', status: 'Absent', time: '10:00 AM' },
      ],
      results: [
        { id: 101, name: "Semester 3 Final Transcript", type: "PDF", size: "1.2 MB", date: "Dec 15, 2025", score: "GPA: 3.8" },
        { id: 102, name: "Data Structures Mid-Term", type: "PDF", size: "0.8 MB", date: "Oct 20, 2025", score: "88/100" }
      ]
    },
    { 
      id: 'S2', name: 'Bob Jones', rollNo: 'CS-2024-002', section: 'A', photo: 'https://i.pravatar.cc/150?u=S2', password: 'password123',
      personalDetails: { email: 'bob@student.com', phone: '+1 987 654 3210', parentPhone: '+1 987 654 3211', address: '456 Student Hostel', dob: '2003-11-22' },
      collegeDetails: { 
        course: 'B.Tech', branch: 'Computer Science', year: '2nd Year', batch: '2024-2028', semester: '4th', advisor: 'Prof. John Doe',
        faculties: [
          { subject: 'Data Structures', name: 'Prof. John Doe' },
    ...
    { 
      id: 'S3', name: 'Charlie Brown', rollNo: 'CS-2024-003', section: 'B', photo: 'https://i.pravatar.cc/150?u=S3', password: 'password123',
      personalDetails: { email: 'charlie@student.com', phone: '+1 555 123 4567', parentPhone: '+1 555 123 4568', address: '789 Off-campus Apt', dob: '2004-01-30' },
      collegeDetails: { 
        course: 'B.Tech', branch: 'Computer Science', year: '2nd Year', batch: '2024-2028', semester: '4th', advisor: 'Prof. Sarah Lee',
        faculties: [
          { subject: 'Operating Systems', name: 'Prof. Sarah Lee' },
          { subject: 'Computer Networks', name: 'Prof. Vint Cerf' }
        ]
      },
      attendance: [], results: []
    }
  ...
  teachers: [
    { id: 'T1', name: 'Prof. John Doe', email: 'john@teacher.com', password: 'password123', department: 'Computer Science', sections: ['A', 'B'] },
    { id: 'T2', name: 'Prof. Sarah Lee', email: 'sarah@teacher.com', password: 'password123', department: 'Mathematics', sections: ['B', 'C'] }
  ]
};
...
export default function App() {
  ...
  const handleLogin = (e) => {
    ...
    if (email.endsWith('@admin.com')) {
      ...
      if (admin && admin.password === loginPassword) {
        ...
        setAuthMessage({ type: 'error', text: 'Invalid admin credentials.' });
      }
    } 
    else if (email.endsWith('@teacher.com')) {
      const teacher = db.teachers.find(t => t.email.toLowerCase() === email);
      if (teacher && teacher.password === loginPassword) {
        setActiveRole('teacher');
        setActiveUser(teacher);
      } else {
        setAuthMessage({ type: 'error', text: 'Invalid email or password for Faculty.' });
      }
    } 
    else if (email.endsWith('@student.com')) {
      const student = db.students.find(s => s.personalDetails.email.toLowerCase() === email);
      if (student && student.password === loginPassword) {
        setActiveRole('student');
        setActiveUser(student);
      } else {
        setAuthMessage({ type: 'error', text: 'Invalid email or password for Student.' });
      }
    } 
    else {
      setAuthMessage({ type: 'error', text: 'Invalid domain. Must use @admin.com, @teacher.com, or @student.com' });
    }
  };
  ...
  const handleRegister = (e) => {
    ...
    // Strict Admin Authorization Check (allows '123456789' or '12345 6789')
    if (adminKey.replace(/\s/g, '') !== '123456789') {
      return setAuthMessage({ type: 'error', text: 'Invalid Master Admin Key. Registration denied.' });
    }

    const email = regEmail.toLowerCase().trim();
    const domain = email.split('@')[1];

    if (domain !== 'admin.com') return setAuthMessage({ type: 'error', text: 'Admin email must end with @admin.com' });

    if (db.admins.some(a => a.email === email)) {
      return setAuthMessage({ type: 'error', text: 'An admin account with this email already exists.' });
    }

    const newAdmin = {
      id: 'A' + Date.now(), name: regName, email: email, password: regPassword
    };
    setDb(prev => ({ ...prev, admins: [newAdmin, ...prev.admins] }));
    
    setAuthMessage({ type: 'success', text: `Admin account created! You can now log in.` });
    setLoginEmail(email);
        ...
        <div className="bg-white max-w-4xl w-full rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="p-6 sm:p-10 md:w-1/2 flex flex-col justify-center bg-white relative">
            ...
            <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
              <div>
                <h2 className="text-2xl sm:text-3xl font-bold text-slate-800 mb-2">{isLoginView ? 'Welcome Back' : 'Admin Register'}</h2>
                <p className="text-slate-500 text-sm">{isLoginView ? 'Please sign in to your assigned account.' : 'Register a new Super Administrator.'}</p>
              </div>
              <div className="flex bg-slate-100 p-1.5 rounded-xl shrink-0 w-full sm:w-auto">
                <button onClick={() => {setIsLoginView(true); setAuthMessage({type:'', text:''});}} className={`flex-1 sm:flex-none px-5 py-2 text-sm font-bold rounded-lg transition-all ${isLoginView ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}>Login</button>
                <button onClick={() => {setIsLoginView(false); setAuthMessage({type:'', text:''});}} className={`flex-1 sm:flex-none px-5 py-2 text-sm font-bold rounded-lg transition-all ${!isLoginView ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}>Register</button>
              </div>
            </div>
  ...
  const logout = () => { setActiveRole(null); setActiveUser(null); setLoginEmail(''); setLoginPassword(''); setIsLoginView(true); setAuthMessage({type:'', text:''})};
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 flex flex-col">
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        {activeRole === 'student' && <StudentDashboard db={db} user={activeUser} />}
        {activeRole === 'teacher' && <TeacherDashboard db={db} setDb={setDb} user={activeUser} />}
        {activeRole === 'admin' && <AdminDashboard db={db} setDb={setDb} />}
      </div>
    </div>
  );
}
...
function StudentDashboard({ db, user }) {
  const [activeTab, setActiveTab] = useState('attendance'); // Defaulting to attendance for better mobile entry
  const [selectedDate, setSelectedDate] = useState(24);

  const currentStudent = db.students.find(s => s.id === user.id);

  // --- DYNAMIC FACULTY COMPUTATION ---
  // Automatically find all teachers whose assigned sections include this student's section
  const assignedFaculties = db.teachers
    .filter(t => t.sections.includes(currentStudent.section))
...
function TeacherDashboard({ db, setDb, user }) {
  ...
  const currentTeacher = db.teachers.find(t => t.id === user.id);
  ...
  useEffect(() => {
    if (currentTeacher.sections.length > 0 && !selectedSection) {
      setSelectedSection(currentTeacher.sections[0]);
    }
  }, [currentTeacher]);
    ...
    setTimeout(() => {
      ...
      setDb(prevDb => {
        const updatedStudents = prevDb.students.map(student => {
          if (student.section === selectedSection && attendanceDraft[student.id]) {
            const existingIndex = student.attendance.findIndex(a => a.date === attendanceDate);
            const newRecord = { date: attendanceDate, subject: newSubject, status: attendanceDraft[student.id], time: '02:00 PM' };
      ...
      setToastMessage("Attendance synced successfully! Changes are live globally.");
      setTimeout(() => setToastMessage(null), 4000);
    }, 3000); // 3s delay for snappier feedback
  };
    ...
    <div className="flex-1 flex flex-col md:flex-row h-full relative">
      ...
      <aside className="w-full md:w-64 bg-white border-b md:border-b-0 md:border-r border-slate-200 flex flex-col shrink-0">
        ...
        <nav className="p-2 md:p-4 flex flex-row md:flex-col gap-2 overflow-x-auto scrollbar-hide">
          {currentTeacher.sections.map(sec => (
...
function AdminStudentForm({ studentId, db, setDb, onBack }) {
  ...
  const handleSave = (e) => {
     ...
     if (isNew) setDb(prev => ({ ...prev, students: [formData, ...prev.students] }));
     else setDb(prev => ({ ...prev, students: prev.students.map(s => s.id === studentId ? formData : s) }));
     setShowToast(true);
